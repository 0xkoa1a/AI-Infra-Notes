...
下图先把 MoonEP 的控制面与数据面串起来。核心是：**同一份当前路由计划，同时决定 token 写到哪里、哪些权重需要临时复制，以及 Grouped GEMM 怎样解释固定 buffer。**
...
.moon-map {
  ...
  border-radius: 1rem;
  background:
    radial-gradient(circle at 5% 0%, color-mix(in srgb, var(--moon-violet) 10%, transparent), transparent 32%),
    radial-gradient(circle at 100% 55%, color-mix(in srgb, var(--moon-blue) 8%, transparent), transparent 30%),
...
.moon-map__section-label span {
  ...
  border-radius: 999px;
  color: white;
  background: var(--moon-violet);
  font-size: 0.68rem;
}

.moon-map__control {
  display: grid;
  grid-template-columns: minmax(0, 0.9fr) auto minmax(0, 1fr) auto minmax(0, 1.35fr);
  align-items: center;
  gap: 0.55rem;
  margin-bottom: 1rem;
}
...
.moon-map__node--router {
  border-color: color-mix(in srgb, var(--moon-teal) 46%, var(--kb-border));
  background: color-mix(in srgb, var(--moon-teal) 8%, var(--kb-surface));
}

.moon-map__node--planner {
  border-color: color-mix(in srgb, var(--moon-violet) 50%, var(--kb-border));
  background: color-mix(in srgb, var(--moon-violet) 9%, var(--kb-surface));
}
...
.moon-map__badge {
  ...
  border-radius: 999px;
  color: var(--moon-violet);
  background: color-mix(in srgb, var(--moon-violet) 12%, transparent);
  font-size: 0.62rem;
  font-weight: 800;
}
...
.moon-map__output {
  ...
  border-left: 3px solid var(--moon-blue);
...
.moon-map__path--token {
  border-top: 3px solid var(--moon-blue);
}

.moon-map__path--weight {
...
.moon-map__gemm {
  ...
  gap: 0.45rem;
  padding: 0.72rem;
  border: 1px solid color-mix(in srgb, var(--moon-violet) 48%, var(--kb-border));
  border-radius: 0.65rem;
  background: color-mix(in srgb, var(--moon-violet) 8%, var(--kb-surface));
}

.moon-map__gemm-center {
  color: var(--moon-violet);
  font-size: 0.75rem;
  font-weight: 800;
  text-align: center;
...
.moon-map__combine .moon-map__node {
  position: relative;
  padding: 0.55rem 0.45rem;
  font-size: 0.68rem;
}

.moon-map__combine .moon-map__node:not(:last-child)::after {
  position: absolute;
  top: 50%;
  right: -0.31rem;
  z-index: 2;
...
.moon-map__fact--overflow {
  border: 1px dashed color-mix(in srgb, var(--moon-orange) 60%, var(--kb-border));
  background: color-mix(in srgb, var(--moon-orange) 7%, var(--kb-surface));
}
...
.moon-map__backward .moon-map__node {
  border-color: color-mix(in srgb, var(--moon-teal) 40%, var(--kb-border));
}

.moon-map__backward-note {
  margin-top: 0.45rem;
  color: var(--kb-muted);
  font-size: 0.66rem;
  line-height: 1.38;
...
@media (max-width: 820px) {
  ...
  .moon-map__control > .moon-map__arrow {
...
<div class="moon-map" role="img" aria-label="MoonEP 核心机制：当前路由驱动规划、Token Dispatch、权重预取、Grouped GEMM、Combine 与训练反向梯度归约">
  <div class="moon-map__eyebrow">MoonEP core architecture</div>
  <div class="moon-map__headline">一份当前精确计划，同时编排 token、权重与固定执行布局</div>
  <div class="moon-map__section-label"><span>1</span>控制面：当前 Router 输出变成三类执行元数据</div>
  <div class="moon-map__control">
    <div class="moon-map__node moon-map__node--router">
      <b>当前 Router 输出</b>
      <small>逐 token Top-k + 本地 <code>tokens_per_expert</code></small>
    </div>
    <div class="moon-map__arrow">→</div>
    <div class="moon-map__node moon-map__node--planner">
      <span class="moon-map__badge">Rank 0 · GPU</span>
      <b>集中在线规划</b>
      <small>各 Rank 发布本地计数；对称内存汇集并分发结果</small>
    </div>
    <div class="moon-map__arrow">→</div>
    <div class="moon-map__outputs">
      <div class="moon-map__output"><b>alloc / dst</b>　每个 token 的目的 Rank 与最终行偏移</div>
      <div class="moon-map__output"><b>experts_to_copy</b>　每个 Rank 要临时借用的远端权重</div>
      <div class="moon-map__output"><b>cu_seqlens</b>　每一行权重对应的 token 分段</div>
    </div>
  </div>
  <div class="moon-map__body">
    <div class="moon-map__panel">
      <div class="moon-map__section-label"><span>2</span>前向数据面：Dispatch 与权重预取并行，随后汇入同一次 GEMM</div>
      <div class="moon-map__paths">
        <div class="moon-map__path moon-map__path--token">
          <b>Token Dispatch · alloc / dst</b>
          源 Rank 直接把 token 写入目的 Rank 的 <code>NvS</code> 最终 Expert 分段，不再在接收端做一次按 Expert permute。
        </div>
        <div class="moon-map__path moon-map__path--weight">
          <b>Weight Prefetch · experts_to_copy</b>
          目的 Rank 从主实例所在 Rank 读取选中的完整权重，填入本地 <code>B</code> 个临时槽位；与 Dispatch 并行但共享 NVLink。
        </div>
      </div>
      <div class="moon-map__converge">↓　token 分段、权重行号与 cu_seqlens 在规划阶段已经对齐　↓</div>
      <div class="moon-map__gemm">
        <div class="moon-map__node">
          <b>固定 token buffer</b>
          <small><code>NvS</code> 行，按 VM group 排列并含 padding</small>
        </div>
        <div class="moon-map__gemm-center">VM Grouped GEMM<br>＋ cu_seqlens</div>
        <div class="moon-map__node">
          <b>连续 <code>[E+B]</code> 权重视图</b>
          <small><code>[0,E)</code> 对称映射主实例；<code>[E,E+B)</code> 本地预取槽位</small>
        </div>
      </div>
      <div class="moon-map__combine-title">3　Combine：结果留在对称通信 shard，token 源 Rank 主动拉回</div>
      <div class="moon-map__combine">
        <div class="moon-map__node"><b>目的 Rank</b><small>FFN 输出留在本地 NVLink shard</small></div>
        <div class="moon-map__node"><b>本地预归约</b><small>重复 Top-k 行以 FP32 累加到代表行</small></div>
        <div class="moon-map__node"><b>跨 Rank 发布</b><small>同步后保证远端 shard 可见</small></div>
        <div class="moon-map__node"><b>源 Rank 远程读取</b><small>按保存的偏移做 K 路求和，直接得到 <code>[S,H]</code></small></div>
      </div>
      <div class="moon-map__route-note">路由权重沿独立路径收集回 <code>[S,K]</code>；当前 Combine kernel 完成的是远程收集、unpermute 与 Top-k 输出求和，不在其中乘路由权重。</div>
    </div>
    <div class="moon-map__aside">
      <div class="moon-map__section-label"><span>!</span>贯穿全程的不变量</div>
      <div class="moon-map__fact">
        <b>真实负载：<code>S × K</code></b>
        每个 Rank 执行完全相同数量的 routed token。
      </div>
      <div class="moon-map__fact">
        <b>物理形状：<code>NvS</code></b>
        加上每个 VM group 的 padding 后，通信与 GEMM buffer 仍是固定大小。
      </div>
      <div class="moon-map__fact">
        <b><code>zero_copy=True</code> 是可选集成</b>
        Dispatch 返回通信 shard 视图，FFN 直接读写；默认模式仍在 FFN 与通信边界执行 copy。
      </div>
      <div class="moon-map__fact moon-map__fact--overflow">
        <b>推理溢出 · 虚线路径</b>
        若远端 Expert 种类超过 <code>B</code>，不再预取；Grouped GEMM 通过 <code>[0,E)</code> 对称映射直接远程读取主实例权重。
      </div>
    </div>
  </div>
  <div class="moon-map__backward">
    <div class="moon-map__section-label"><span>4</span>训练反向：副本梯度由主实例所在 Rank 拉回</div>
    <div class="moon-map__backward-flow">
      <div class="moon-map__node"><b>预取槽位产生梯度</b><small>写入框架不可见的独立 reduce buffer</small></div>
      <div class="moon-map__node"><b>主实例所在 Rank 远程拉取</b><small>读取所有 Rank 中属于自己 Expert 的槽位</small></div>
      <div class="moon-map__node"><b>累加主梯度</b><small>恢复与无临时副本时相同的参数梯度</small></div>
      <div class="moon-map__node"><b>各 Rank 本地清零</b><small>清空自己的 reduce buffer，供下一 microbatch 复用</small></div>
    </div>
    <div class="moon-map__backward-note">训练要求 <code>B = E/R</code>，不允许走远程溢出权重路径；否则本地没有可控槽位承接对应的权重梯度。</div>
  </div>
</div>
...
**预取能与 dispatch 重叠，也只能与它重叠。** 两者都排在规划之后、GEMM 之前，且互不依赖——一个搬 token，一个搬权重。MoonEP 把它们放在通信流上异步发起，靠事件在 GEMM 前汇合。
