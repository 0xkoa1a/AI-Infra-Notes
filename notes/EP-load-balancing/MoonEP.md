...
MoonEP 是 Moonshot AI 的 EP 通信与执行系统。它使用对称内存，让每个 Rank 可以通过一致的地址布局访问本地或远端的通信 buffer、Expert 权重与梯度 buffer。

## 动机

**MoonEP 实现了 rank 间的完美负载均衡**，所有 rank 的负载严格相等。这使得 MoonEP 拥有静态 shape 与零拷贝。支撑这个目标的是三件互相咬合的事：
...
MoonEP 的控制面与数据面共用同一份当前路由计划：它同时决定 token 的目的地、需要临时预取的远端 Expert，以及固定 buffer 中各段应使用的权重。
...
分配表由两级贪心求出：第一级决定 Rank 之间收发多少 token，把均衡做到底；第二级决定这些额度由哪些 Expert 出，优化的是要借的权重份数。
