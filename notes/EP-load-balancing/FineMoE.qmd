---
title: "FineMoE：在 EDP 副本之间调度 Token"
order: 7
---

FineMoE 的核心不是为每个 microbatch 迁移专家，而是利用数据并行已经产生的同专家副本，在一个更大的通信组内重新安排 token。这样，逐 microbatch 变化的是 token 去哪个现有副本，专家参数的位置只在更长的时间尺度上调整。

论文中的两个名字分工不同：

- **FineEP** 是并行与 token scheduling 策略，定义可用哪些专家副本以及怎样在它们之间分流
- **FineMoE** 是建立在 FineEP、Megatron-LM 和 placement manager 之上的训练系统，负责执行调度、维护布局并在必要时迁移模型状态

论文的部分图表和残留文字仍使用旧称 **MicroEP/MicroMoE**。本文统一写作 FineEP/FineMoE；引用原图时，图中的旧称指向同一方案。

## 从 Vanilla EP 到 FineEP

FineEP 的调度空间来自 Expert Data Parallelism（EDP），不是临时创建的新副本。它成立需要三个前提：

- DP degree 大于 EP degree，因此一个 DP group 内包含多个 EP group
- 同一个逻辑专家在不同 EP group 中已经有多份等价参数，这些副本通过 EDP 同步
- 每张 GPU 承载多个本地专家实例，placement 才有在 GPU 之间打散的空间

下图的四卡例子把调度空间如何一步步扩大画得很清楚。形状表示 token 的源 GPU，颜色表示 Router 选中的专家；底部同色曲线连接同一专家的 EDP 副本。图中 `MicroEP` 是 FineEP 的旧称。

![FineEP 调度空间：从 Vanilla EP、合并 EP group 到打散 placement（论文 Figure 3）](images/finemoe-scheduling-space.png)

- **Vanilla EP**：每个 token 只能发给自己所在 EP group 内的目标专家。Router 结果确定专家，固定 placement 又唯一确定该专家所在的 GPU，因此没有副本选择空间。图中两个 EP group 的最大负载分别为 12 和 10
- **只合并 EP group**：token 可以在同一专家的 EDP 副本之间转移，但各 EP group 仍使用相同 placement。于是不同专家的 EDP group 要么完全相同、要么互不相交：图中 $E_0/E_1$ 只能在 GPU 0、2 之间均衡，$E_2/E_3$ 只能在 GPU 1、3 之间均衡。局部最优以后，全局最大负载仍是 11
- **FineEP**：在合并后的 EP group 内打乱专家 placement，使不同专家的 EDP group 相互交叉。一个专家沿自己的副本集合转移负载，另一个专家再沿另一组交叉的副本继续转移；多条路径连起来后，负载可以传播到整个 FineEP group，例子中四张 GPU 都降到 8
...
因此 FineEP 做的是**在已有 EDP 副本上扩大 token 的调度空间**。它没有为当前 microbatch 复制或迁移专家，也没有改变 Router 为 token 选择的逻辑专家；变化的只是由哪个等价副本执行计算。
...
- placement 在相邻多个 microbatch 之间不变，所以约束矩阵不变；变化的主要是每个专家的 $\mathrm{load}_e$
...
然后按两个阶段生成 token-to-replica mapping：
...
实现操作的是连续 token range，而不是逐 token 做一次调度决定。这样生成的结果仍然精确满足 LP 配额，但元数据量和控制开销更小。固定遍历顺序也让所有 GPU 在相同输入下能独立重建同一份确定性方案。

这里要区分三个层次的优化目标：

- **基础 LP** 只最小化最大计算负载，不关心 token 从哪里来
- **locality-aware routing** 不改变 LP 给定的副本配额，只在实现这些配额时优先保留本地 token，从而减少实际跨卡流量
...
- 每张 GPU 在 CPU 上独立运行相同的 LP 和 range routing 算法
...
LP 只能在 $\mathcal G_e$ 中分配专家 $e$ 的 token，因此 placement 直接决定 LP 的约束矩阵与可行域。FineMoE 用图来表达这种限制：

- 每张 GPU 是一个顶点
- 一个专家连接所有持有其副本的 GPU，因此是一个 hyperedge
- 当一个专家恰好有两个副本时，hyperedge 就退化成普通边
- 专家的 token 总数是这条边的权重；token 只能沿同一条边所覆盖的顶点重新分配

![FineMoE 的 placement 图抽象：GPU 是顶点，专家是边（论文 Figure 5）](images/finemoe-placement-graph.png)

理解 placement 瓶颈的关键，是看“负载为什么会困在一组 GPU 里”。假设一组 GPU 最终都达到了最优解中的最大负载：

- 如果某个专家在这组 GPU 外还有副本，它的 token 就能继续向外转移；在论文选择的最优解中，不必让它在这组最忙 GPU 上留下负载
- 真正无法流出的，是副本全部落在这组 GPU 内的专家。无论 LP 怎样分配，这些专家的全部 token 都只能由组内 GPU 承担
- 因而这组 GPU 的平均负载不可能低于“困在其中的专家总负载 ÷ GPU 数量”
...
这里的“density”不是泛指每张 GPU 放了几个专家，而是只计算**整个副本集合都被封闭在子集 $S$ 内**的专家负载。它给 placement 一个比“每卡专家数相等”更精确的目标：避免某个小 GPU 子集封闭地持有过多高负载专家，使大量 token 无处可去。好的 placement 要让所有诱导子图中最坏的 trapped-load density 尽量低。

## 两种 Placement 与自适应更新

### 无负载先验：对称 placement

模型初始化时还不知道真实专家负载，FineMoE 把所有专家视为服从相同分布，先构造对称 placement。论文使用 Cayley graph，核心直觉是：
...
如果已经观察到稳定的负载差异，再要求所有专家副本数和位置完全对称就会浪费容量。FineMoE 用两步经验策略构造针对当前负载的非对称 placement：
...
- **再决定副本位置**
  - 随机生成多份满足每卡容量与正确性约束的 placement graph
  - 用最大诱导子图 density 评价每份候选
  - 选择最坏 density 最低的一份

第一步解决“热专家应该有几份”，第二步解决“这些副本怎样分散才不会形成封闭热点”。Monte Carlo sampling 扩大了搜索范围，但整套方法仍是经验性的近优策略，不应写成精确求得全局最优 placement。

### Placement 的正确性约束

打乱 placement 后，不同专家会使用不同的 EDP group。训练中的参数与梯度同步如果以不一致顺序进入这些通信组，可能互相等待并死锁。
...
- **每个 microbatch**：placement 固定，LP token scheduling 根据当前 Router 结果重新分配副本配额，处理瞬时波动
- **每隔若干 iteration**：placement manager 在后台汇总历史负载，用 moving average 等时间序列方法预测后续分布，再用最大诱导子图 density 评价当前 placement
- **预测性能低于阈值时**：生成新的非对称 placement，并重新初始化全局模型状态，把参数和 optimizer state 迁移到新位置

Replacement 会暂停训练，而且需要搬动远大于 token 元数据的模型状态，所以只能低频执行。它是针对极端或持续 skew 的粗粒度补充；FineMoE 的主要均衡手段仍是固定 placement 上的逐 microbatch token scheduling。

## 与 LPLB 的区别

FineMoE 与 `LPLB.qmd` 都把在线分流写成 LP，但两者的副本来源、可行域和求解路径不同：

| 维度 | FineMoE | LPLB |
| --- | --- | --- |
| 副本来源 | DP 中多个 EP group 已有的 EDP 副本 | EPLB 预先建立的主专家—副本关系 |
| 拓扑 | 一般 graph/hypergraph，placement 可重排 | 预设的成对共享图 |
| LP 变量 | 每个专家在各副本上的 token 配额 | 每条主副本边两端的分流比例 |
| 求解 | CPU HiGHS、warm start、每卡独立求解 | GPU 小型内点法、固定迭代并带 fallback |
| Placement | 用图论分析对称/非对称布局，并可周期替换 | 在线阶段拓扑固定，位置不变 |
...
- FineEP 的 LP 求解与 token routing 合计最低约 $100\,\mu s$；即使扩展到 64 GPU、256 experts，仍低于 1 ms
- warm start、overlap 与 locality-aware routing 组合后，论文的示例中 Dispatch 相比 Megatron-LM 只增加约 0.4 ms
- adaptive replacement 迁移专家参数和 optimizer state 通常需要数百毫秒，因此论文建议训练早期约每 50 iteration 检查一次；进入稳定阶段后放宽到数百步，甚至停止 replacement
