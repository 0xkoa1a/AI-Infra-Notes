...
这份 Level 9 实现让不同 warp 分别负责输入加载、矩阵计算的发射和结果写回，并通过完成通知交接缓冲。**读懂它的关键，是弄清每份数据由谁持有、何时完成，以及何时允许下一轮覆盖同一块存储。**

本文精读 [MatmulTutorial 的 Level 9 源码](https://github.com/KnowingNothing/MatmulTutorial/blob/69c6886b93c3d569f1ba49185615c7fce9f19df0/examples/matmul/this-sm100/level9/matmul.cu)，源码链接固定到 commit `69c6886`。下面的 C++ 片段保留实现中的控制关系，调整了断行并补充中文注释；未展开的指针设置和 helper 可对照原文件与文末附录。性能数字来自同版本作者报告，未在本文中复测。

## 从普通 GEMM 到 Blackwell 的执行路径

本文沿用分块 GEMM 的计算任务：两份 BF16 输入相乘，以 FP32 累加，最后写出 BF16 结果。

$$
D=AB^{\mathsf T}.
$$

- $M$、$N$：输出的行数与列数；$K$：相乘求和的维度。
- $A$：行主序 $[M,K]$，$B$：行主序 $[N,K]$；计算时逻辑转置 B。
- $D$：行主序 $[M,N]$。下文用 tile 表示一个 CTA 负责的输出块。

### 搬运、计算与累加结果分别由谁负责

这份实现中的三项机制决定了代码的数据路径：

| 机制 | 本实现中的职责 | 线程负责什么 |
|---|---|---|
| TMA（Tensor Memory Accelerator） | 按描述好的布局，在全局内存与共享内存之间搬运数据 | 提交加载或写回指令，并按需等待完成 |
| UMMA | 在 Tensor Core 上执行异步矩阵乘加，将结果累加到 TMEM | 由专门的 warp 选一个线程发射指令 |
| TMEM（Tensor Memory） | 保存 FP32 累加结果 | Epilogue 线程用专门的加载指令读入寄存器，再转换为 BF16 |

TMA 和 UMMA 的发射线程可以在指令提交后继续执行。**发出指令时，搬运或计算可能尚未完成**，因此下一角色需要等完成通知，才能读取结果或覆盖旧数据。后文的 `full` 表示数据已可读，`empty` 表示旧数据已用完、允许覆盖；“归还缓冲”也是允许覆盖的意思。

完成通知主要通过共享内存中的 `mbarrier` 传递。`arrival` 是向 barrier 报到；TMA 加载还会登记并完成一定字节数的传输。`phase` 区分同一个 barrier 的连续轮次。线程等到对应轮次完成后，配合 fence 建立所需的操作顺序，再继续访问数据。具体计数与 fence 放在对应代码处解释。

### 两个 CTA 如何协作

此实现用两个 CTA 组成一个 cluster，共同执行 `cta_group::2` UMMA。CTA 0 是 cluster 内的 leader，负责发射矩阵指令。两边各自加载不同 M 行的 A；B 沿 N 分半加载，而两边的输出都需要这两半 B。

<BlackwellGemmDiagram view="cluster" />

每个 CTA 最终负责一个 $256\times256$ 输出 tile。沿 K 每次取 64 个元素，称为一个 **K block**：各 CTA 加载 A $[256,64]$ 和 B $[128,64]$，经过多轮累加完成输出。

> 这里先假定配对 CTA 负责相同 N 区间。后文“Persistent scheduler 与配对条件”给出这一前提的限制和反例。

### 数据经过哪些缓冲

每个 CTA 有四份 A/B 输入缓冲、一份 TMEM 累加区，以及两份输出暂存区。代码把输出暂存区称为 **CD SMEM**。输入按 K block 交接；TMEM 保留完整 tile 的累加值；写回时再把 tile 切成 $[128,64]$ 的 chunk，交替使用两份 CD 缓冲。

<BlackwellGemmDiagram view="storage" />

四个输入 stage 允许加载与矩阵计算按不同进度推进。TMEM 只有一份，下一 tile 必须等旧结果读完才能使用它。CD 的两份缓冲则让线程填充新 chunk 时，TMA 有机会同时写回前一个 chunk。下面先跟完一个 tile，再讨论跨 tile 的调度与重叠。

## 沿一个输出 tile 阅读代码

### 先定位角色和循环层次

每个 CTA 有 8 个 warp。源码通过 `warp_idx` 分支分配工作；其中 “MMA warp” 是发射 UMMA 的线程角色，epilogue 指累加结束后的转换与写回阶段。
...
三个角色分别创建 `TileScheduler`，按相同规则获得 `(m_block, n_block)`。各自的 `while (scheduler.get_next_block(...))` 每次处理一个输出 tile；它们可以处于不同执行进度，由缓冲的完成通知协调。初始化位于这些循环之前，资源释放位于循环退出之后。

| 源码位置 | 每次处理什么 | 本节展开的位置 |
|---|---|---|
| TMA：`if (warp_idx == 0 && elect_one())` | 外层 `while` 取 tile；内层 `kb` 循环加载输入 | TMA producer |
| MMA：`else if (warp_idx == 1 && is_leader)` | 外层 `while` 取 tile；`kb` 遍历 K，再由 `k`、`w` 发出 UMMA | MMA consumer |
| Epilogue：`else if (warp_idx >= 4)` | 外层 `while` 取 tile；`w`、`s` 遍历输出 chunk | Epilogue |

### 配置：一次计算覆盖多大范围

每个 K block 有 64 个 K 元素，一次 UMMA 推进 16 个，因而分成四个 **K-step**。每次 2CTA UMMA 在每个 CTA 产生 128 行局部累加值；每边的 256 行输出需要两个 M-wave，代码用 `w=0,1` 选择它们。
...
指令的 `UMMA_M=256` 合计两 CTA 各 128 行；`BLOCK_M=256` 则是一个 CTA 的完整输出行数。两个常量恰好相等，但统计范围不同。

### 初始化：发布 barrier，分配 TMEM

**执行位置：三个角色的 tile 循环之前，只初始化一次。** 每个 CTA 都有自己的 SMEM 和 barrier 副本。Leader 的 MMA warp 等待 leader 上的 `full_bar` 和 `tmem_empty_bar`；TMA 和 epilogue 则分别等待本地的 `empty_bar` 和 `tmem_full_bar`。
...
`fence_barrier_init()` 使本线程的 mbarrier 初始化对后续异步操作可见，`cluster_sync()` 让整个 cluster 集合后再进入工作阶段。这两个调用共同保证后面的跨 CTA 通知有已初始化的目标。Barrier 和 TMEM 的初始化都在 persistent loop 之外，后续通过 phase 区分轮次。
...
**执行位置：每个 CTA 的 warp 0，先遍历输出 tile，再遍历该 tile 的 K block。** 以下摘录保留真实循环与状态变量，循环体在下一段展开：
...
while (scheduler.get_next_block(m_block, n_block)) {
    ...
    const int32_t n_coord = n_block * BLOCK_N
        + cta_rank * LOAD_N_PER_CTA;
...
选中的线程先等待本地 `empty_bar[stage]`，保证上次 UMMA 已经用完这一格缓冲；随后向 leader 的 `full_bar[stage]` 登记本轮加载。
...
`num_k_blocks=ceil(K/64)`；A 的起始行是 `m_block * 256`，B 的起始 N 行是 `n_block * 256 + cta_rank * 128`。每个 CTA 加载 48 KiB，两个 CTA 合计 96 KiB；leader 登记这份合计字节数，每个 CTA 各贡献一次 arrival。只有 arrival 与 TMA 的字节完成条件都满足，本轮 `full_bar` 才完成。
...
Leader 的 warp 1 全部参与等待与 shuffle；选中的一个线程发射 UMMA 和 commit。先明确结果放在哪里，再沿外层 tile 循环进入 K block。

每个 CTA 的逻辑输出有 256 行，而 TMEM 提供 128 条 lane。这里的 lane 是 TMEM 的行索引，范围为 0–127；代码中的 `lane_idx` 是 warp 内线程编号，范围为 0–31。每个 TMEM 单元保存一个 32-bit 累加值。

实现把输出按行分为两个 **M-wave**，每个 wave 有 128 行。它们使用相同的 TMEM lane 范围，但占用不同的 column 区间：
...
两个 wave 的结果同时保留。MMA 代码中的 `w * BLOCK_N` 选择不同 column 区间；它们合计占用 `128 × 512 × 4 = 256 KiB` TMEM，形成一个 accumulator stage。

**一次准备：缓存四个 stage 的 descriptor。** SMEM descriptor 告诉 UMMA 从哪个共享内存地址、按什么布局读取 A/B。它的低位包含以 16 bytes 为单位的起始地址，高位保留布局信息；这与 TMA 搬运使用的 tensor map 是不同的描述信息。Warp 内线程 0–3 各存一个 stage 的低位，计算时通过 shuffle 广播当前 stage 的值：
...
**每个输出 tile：先等 TMEM 归还。** TMEM 只有一个 accumulator stage，所以 `accum_idx` 总为 0；`accum_phase` 每个输出 tile 翻转一次。下一 tile 必须等两个 CTA 的所有 epilogue 线程完成旧结果读取。

下面的循环同时标出 tile 开始时的等待，以及每个 K block 开始时的等待：
...
while (scheduler.get_next_block(m_block, n_block)) {
    ...
    for (uint32_t kb = 0; kb < num_k_blocks; ++kb) {
        ...
        tcgen05_fence_after();
        // 此处省略 descriptor 选择、UMMA、commit 与 stage/phase 更新；
        // 以下片段依次展开这些操作。
    }
}
```

等到输入就绪后，由 `stage` 选出保存 descriptor 的线程：
...
**每个 K block 结束：登记 UMMA 完成通知。** 发出 UMMA 后不会立即覆盖 A/B。Commit 为此前发出的异步计算登记完成通知；这些计算完成后，向两个 CTA 的 `empty_bar` 各发送一次 arrival；最后一个 K block 还要通知输出结果就绪。
...
**执行位置：每个 CTA 的 warp 4–7，共 128 个线程。** Epilogue 先等待完整 tile 的 UMMA 累加完成。它们按 `w=0,1`、每个 wave 内 `s=0,1,2,3` 的顺序处理 8 个 $[128,64]$ chunk。**每个 chunk 搬到 CD SMEM 后就发出对应 Store，边搬边写回。**
...
这三个变量在 epilogue 的 tile 循环外定义。`local_tid` 的范围为 0–127，每个线程读取一条 TMEM lane；输出行由 `m_block * 256 + w * 128 + local_tid` 确定。下面保留输出循环，省略的 `accum_idx`、`accum_phase` 计算与 MMA 相同：

```cpp
while (scheduler.get_next_block(m_block, n_block)) {
    // 此处省略 accum_idx、accum_phase 的计算。
    barrier_wait(tmem_full_bar[accum_idx], accum_phase);
    tcgen05_fence_after();
    for (uint32_t w = 0; w < NUM_M_WAVES; ++w) {
        for (uint32_t s = 0; s < NUM_STORES; ++s) {
            // 一个 [128,64] chunk；以下依次展开此处省略的循环体。
        }
    }
}
```

**每个输出 chunk：先等 CD stage 可覆盖。** 只有负责 Store 的线程等待自己的 bulk groups，然后通过 named barrier 让其余 epilogue 线程一起继续：
...
TMA Store 从 CD SMEM 读取已转换的结果。Store 发射后翻转 `tma_store_stage`，下个 chunk 使用另一块缓冲；再次使用原缓冲时，仍需先执行前面的 `wait_group<1>`。

### 排空流水线与释放资源

**执行位置：所有输出 tile 都处理完，退出外层循环之后。** 此时没有“下一 tile 开始时”的等待来保护资源。MMA 分支因此补等最后一轮 `tmem_empty`；epilogue 的 Store 发射线程等待全部 Store 完成。它们分别保护 TMEM 生命周期和最终输出写回。
...
读完三个角色后，可以按“谁最后一次读取、谁下一次覆盖”检查每种缓冲。A/B stage 在该 K block 的 UMMA 完成后即可归还；TMEM 要保留到整块结果被 epilogue 读完；CD 则保留到对应 TMA Store 不再读取它。
...
图中的“chunk 0…6”合并展示前七次搬出过程，包含中途等待 CD stage 的时间；上方只选画其中一笔较早的 Store。最后一个 chunk 的 TMEM 读取全部结束后，MMA 可以开始下一 tile，旧 tile 的末笔 Store 仍可继续。若 CD Store 阻塞了最后一批读取，也会推迟 TMEM 的归还。

### Persistent scheduler 与配对条件

Host 启动有限数量的 CTA，让它们通过前面的外层 `while` 反复处理 tile，这就是 persistent 执行。`TileScheduler` 按确定性的编号计算分配工作。
...
**各角色独立推进同一序列。** TMA、MMA、epilogue 在同一个 CTA 中枚举相同序列，却可以推进到不同位置：TMA 正在预取后续 K block 时，MMA 可能仍在计算当前 tile，epilogue 仍在处理前一个结果。它们分别持有线程私有状态，由 barrier 对齐数据依赖。
...
四份输入 stage、两份输出暂存区和同步对象合计占用约 224 KiB SMEM：
...
A stages 和 B stages 各自连续排列。相邻 stage 的地址步长固定，MMA warp 因而可以用各 lane 保存 descriptor，再通过 shuffle 取出当前 stage 所需的值。TMEM 单独分配，不计入这张 SMEM 预算表。

Level 9 把 tile 扩大到 $256\times256$，减少需要调度的 tile 数，但也占用了全部 512 个 TMEM column，因此只保留一个 accumulator stage。它通过 CD 双缓冲和提前归还 TMEM 获得部分跨 tile 重叠。

<details>
<summary>参考：Level 2–9 的机制演进</summary>
...
| `umma_commit_2sm()` | 登记此前 UMMA 的完成通知，完成后向两边发送 arrival |
