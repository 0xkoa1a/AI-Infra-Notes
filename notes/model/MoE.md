...
自然语言来说，一个 Expert 内部依次完成：

1. $W_1$ 根据输入产生 gate branch。
2. $W_3$ 根据输入产生 value/up branch。
3. Gate branch 经过 SiLU 激活。
4. 两个 branch 逐元素相乘。
5. $W_2$ 将中间表示投影回 hidden dimension。

通常 $W_1$ 和 $W_3$ 接收相同输入，因此可以把它们理解成同一个阶段中的两条并行 projection。
...
自然语言来说，$g$ 决定哪些中间特征应该被激活，而 $u$ 提供这些特征对应的值。
...
自然语言来说，gate branch 对 up branch 进行逐元素调制。
...
将 intermediate representation 投影回 Transformer hidden dimension。

因此一个 SwiGLU Expert 的维度变化通常可以概括为：

$$
d
\xrightarrow{\text{gate/up projection}}
2d_{\mathrm{ff}}
\xrightarrow{\text{SwiGLU}}
d_{\mathrm{ff}}
\xrightarrow{\text{down projection}}
d
$$

这里前面的 $2d_{\mathrm{ff}}$ 来自同时产生 gate 和 up 两个 $d_{\mathrm{ff}}$ 向量。

## L1 与 L2

有些 MoE 工作会把 SwiGLU Expert 中的两级线性计算简称为 L1 和 L2。

这里的 L1/L2 指的是 **Linear-1 和 Linear-2**，不是两级 Router。
...
* **L1**：计算 $W_1x$ 和 $W_3x$。
* **SwiGLU**：计算 $\operatorname{SiLU}(W_1x)\odot W_3x$。
* **L2**：计算最后的 $W_2h$。
...
因此，L1 和 L2 只是对普通 SwiGLU FFN 两级 projection 的命名，并不是 MoE 特有的算法结构。
