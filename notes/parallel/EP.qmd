...
负载均衡问题本身（负载的三重可变性、优化目标、纯重排的下界、硬件前提、四个工作的流变）见 `../EP-load-balancing/background.qmd`；四条具体路线分别见 `../EP-load-balancing/EPLB.qmd`（基于历史统计、周期性离线求解）、`../EP-load-balancing/UltraEP.qmd`（基于精确负载、逐层实时求解）、`../EP-load-balancing/MoonEP.qmd`（把每 Rank 负载钉成常量的 EP 通信库）与 `../EP-load-balancing/LPLB.qmd`（离线定副本拓扑、逐 batch 解 LP 定分流）。
