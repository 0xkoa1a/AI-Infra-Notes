...
class FlashAttentionForwardSm100:
    ...
    def kernel(
        ...
        # v: 一条 cooperative UMMA 指令内部的 CTA 编号。
        # 两个 CTA 共享一个外层 (m=256, n=128, k) tile
        # v=0 和 v=1 两个 CTA 分别负责 128 行
        cta_layout_vmnk = cute.tiled_divide(
            cute.make_layout(self.cluster_shape_mnk), (tiled_mma_qk.thr_id.shape,)
        )
        ...
        if const_expr(self.use_clc_scheduler):
            if warp_idx == self.clc_scheduler_warp_id:
                ...
                if is_leader_cta:
                    # CLC 是 Blackwell 的动态 persistent tile 调度机制。
                    # leader CTA 的 scheduler warp：
                    # 请求下一块 tile → 等待并消费调度结果 → 请求下一块……
                    self.clc_scheduler_warp(tile_scheduler)
                    # CLC pipeline 把 CTA 中所有 warp 都计为消费者。
                    # Load、MMA、Softmax 等计算 warp 会在自己的主循环末尾推进 scheduler
                    # 没有主循环的 empty warp 也必须用这个空循环逐轮跟进
                else:
        ...
        if warp_idx >= self.load_warp_ids[0] and warp_idx <= self.load_warp_ids[-1]:
            # 将当前 warp 的“每线程最大寄存器数”降低到 self.num_regs_other
            # 为 softmax warp 省寄存器
            cute.arch.setmaxregister_decrease(self.num_regs_other)
            self.load(
                thr_mma_qk,
        ...
        if warp_idx == self.mma_warp_id:
            ...
            tmem.wait_for_alloc()
            # 从 SMEM holding buffer 取出 TMEM 指针
            tmem_ptr = tmem.retrieve_ptr(self.qk_acc_dtype)
            # 在这块 TMEM 上运行完整流水线：QK MMA 写入 S，Softmax 读取 S 并生成 P，PV MMA 用 P 和 V 累加 O，Correction 最后读取和修正 O。
            # 这里 tmem_ptr 没显式传入 mma()，是因为该路径固定申请全部 512 columns，源码因此知道起始 column 恒为 0，提前用以 0 为基址的 layout 构造了 tStS/tOtO/tOrP；pointer 仍然参与 allocator 协议和最终释放。
            self.mma(
                tiled_mma_qk,
                tiled_mma_pv,
                sQ,
            ...
            # 通知硬件“本 CTA 此后不会再次申请 TMEM”，并没有释放当前区域
            tmem.relinquish_alloc_permit()
            # 等待 MMA、两组 Softmax 和 Correction 全部停止访问 TMEM
            tmem_alloc_barrier.arrive_and_wait()
            # 真正 free TMEM 的 512 列
            tmem.free(tmem_ptr)
        ...
        #  Softmax
        # ///////////////////////////////////////////////////////////////////////////////
        # stage 0 和 1 分别交给两组 softmax warps
        # softmax warps 的 ID 被刻意安排成从 0 开始的一段连续区间
    ...
    def load(
        ...
        # 这是 persistent kernel 的外层循环。
        work_tile = tile_scheduler.initial_work_tile_info()
        while work_tile.is_valid_tile:
            # split_idx：SplitKV 时当前 CTA 负责的 K/V 区间；普通 Attention 恒为 0。
            ...
            if const_expr(not self.use_block_sparsity):
                ...
                if const_expr(not self.is_split_kv) or n_block_min < n_block_max:
                    ...
                    if issue_kv_for_this_warp:
                        ...
                        kv_producer_state.advance()
                    # 现在沿着序列维度继续 load 后续的 K/V
                    # 刚刚已经 load 了 n_block_max - 1，现在从 n_block_max - 2 开始继续倒序遍历，直到 n_block_min
                    # 对普通 dense attn 来说，这个区间就是整个 K/V 序列
    ...
    def mma(
        ...
        while work_tile.is_valid_tile:
            ...
            if process_tile and is_leader_cta:
                ...
                # 进入循环，每个迭代：
                # P(i-1)·V(i-1) → 更新部分累加 O(i-1)
                # Q·K(i) → 生成 S(i)
                # 通过 S_full 通知 softmax warp 读取 S(i)，生成 stats 和 P(i)
                # softmax warp 生成 stats 后通过 stats_ready 通知 correction warp 读取 stats，对 O(i-1) 进行缩放修正
                # 所以这里算出 O(i-1) 后不需要通知 correction warp。
                ...
                for stage in cutlass.range_constexpr(self.q_stage):
                    ...
                    # 最后一轮 PV 后没有下一次 QK 可以间接证明 O 已完成，所以 Tensor Core 会单独发布 final-O-ready。Correction 等到该事件，使用最终 row sum 归一化 O、做类型转换并写入对应的 SMEM output slot
                    pipeline_o_acc.producer_commit_w_index(stage)
                    # End of GEMM_PV00 (P0 * V0 -> O0_partial)
                P_full_O_rescaled_phase ^= 1
                # 5. release Vi_end
